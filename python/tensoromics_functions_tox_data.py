...
def read_gene_ids_from_tsv_file(filename,n_genes, gene_ids_len, n_header_rows, gene_col):
    ...
    gene_ids_array = _create_empty_c_char_matrix(n_genes, gene_ids_len)
    ...
    return _c_char_matrix_to_strings(gene_ids_array)
...
def read_expression_vectors_tsv(file_list, gene_ids, n_samples, n_header_rows,
    ...
    file_list_matrix = _strings_to_c_char_matrix(file_list)
    ...
    value_cols_ct = np.asfortranarray(value_cols, dtype=np.int32)
...
def read_orthofinder_file(filename, gene_ids, family_ids_len, n_families):
    ...
    # Prepare output arrays
    family_ids_matrix = _create_empty_c_char_matrix(n_families, family_ids_len)
    ...
    _readonly(gene_to_fam)

    return {
        'family_ids': _c_char_matrix_to_strings(family_ids_matrix),
...
def validate_string_array_uniqueness(strings):
    ...
    """
    gene_ids_raw = _strings_to_c_char_matrix(strings)
    ierr = ctypes.c_int()
...
def validate_data_structure(n_genes, n_families, n_samples, gene_ids, gene_family_ids, gene_to_fam, expression_vectors, family_centroids, shift_vectors):
    ...
    gene_ids_raw = _strings_to_c_char_matrix(gene_ids)
    gene_family_ids_raw = _strings_to_c_char_matrix(gene_family_ids)
...
def create_zip_archive(zip_filename: str, keys, filenames) -> None:
    ...
    keys_array = _strings_to_c_char_matrix(keys)
    filenames_array = _strings_to_c_char_matrix(filenames)
