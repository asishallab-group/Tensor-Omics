...
def test_paralog_functions():
    ...
    dosage_mask, = filter_paralogs_by_pattern_dosage_effect(angles, threshold, gene_to_fam, n_families)
    assert (dosage_mask == [7]).all(), "dosage_mask should be [7], got " + str(dosage_mask)

    subfunc_mask, = filter_paralogs_by_pattern_subfunctionalization(angles, threshold, gene_to_fam, n_families)
    assert (subfunc_mask == []).all(), "subfunc_mask should be [], got " + str(subfunc_mask)

    # Testing Work Array Size Calculation

    max_subset_size = 3
    work_array_size, actual_max_subset_size = calc_work_arr_paralog_subsets_size(max_subset_size, n_paralogs, dosage_mask).values()
...
def test_detect_neofunctionalization():
    ...
    # Normalize ancestors
    ancestors = np.apply_along_axis(normalize_unit_length, axis=0, arr=ancestors)
