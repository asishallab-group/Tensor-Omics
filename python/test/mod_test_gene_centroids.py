...
def test_invalid_input_arguments():
    ...
    try:
        group_centroid(np.empty((0, n_genes)), gene_to_family, n_families, mode)
        assert False, "Expected ValueError for n_axes=0"
    except Exception:
        pass
    # Invalid n_genes (empty gene set)
    try:
        group_centroid(np.empty((n_axes, 0)), np.array([], dtype=np.int32), n_families, mode)
        assert False, "Expected ValueError for n_genes=0"
    except Exception:
        pass
    # Invalid n_families (0 families)
    try:
        group_centroid(vectors, gene_to_family, 0, mode)
...
def test_invalid_family_mapping():
    ...
    mode = 'group_all'
    try:
        group_centroid(vectors, gene_to_family, n_families, mode)
