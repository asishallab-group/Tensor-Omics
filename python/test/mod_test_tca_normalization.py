...
def test_tox_normalize_variable_timeseries():
    ...
    v, status = tox_normalize_variable_timeseries(v).values()
...
def test_error_handling():
    ...
    assert_error(lambda: tox_normalize_variable_timeseries(v), "Should raise RuntimeError for NaN")

    # -------------------------------
    # Case 2: Infinity input
    # -------------------------------
    v = np.array([1.0, 2.0, np.inf, 4.0], dtype=np.float64)
    assert_error(lambda: tox_normalize_variable_timeseries(v), "Should raise RuntimeError for infinity")

    # -------------------------------
    # Case 3: Empty array
    # -------------------------------
    v = np.array([], dtype=np.float64)
    assert_error(lambda: tox_normalize_variable_timeseries(v), "Should raise RuntimeError for empty array")
