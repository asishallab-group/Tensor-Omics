...
function get_flags() {
  ...
  # Detect compiler and choose appropriate profile:
  if [[ "$FC" == "ifx" || "$FC" == "ifort" ]]; then
    echo "-O2 -fopenmp-target-do-concurrent -warn all -diag-enable=all -qopenmp -xHost -align array64byte -qopt-zmm-usage=high -qopt-prefetch=3 -qopt-matmul -fPIC -qmkl"
  elif [[ "$FC" == "nvfortran" ]]; then
    echo "-O2 -Mconcur -fPIC -fopenmp -stdpar=multicore"
  else
    echo "-O2 -march=native -mtune=native -fopenmp -funroll-loops -ftree-vectorize -fPIC -llapack -lblas --debug"
  fi
}
