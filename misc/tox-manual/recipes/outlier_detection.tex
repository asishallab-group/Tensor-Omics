...
\begin{workflow}
  ...
  \step{Call \code{detect\_outliers}. It does \emph{not} compare distances to a
        ...
        $1-\optn{quantile\_level}$ of those. Scaling per family is what puts a
...
result = detect_outliers(
    ...
    quantile_level=0.95)               # FRACTION: top 5%
is_outlier = result["is_outlier"]
# an effect size, NOT a p-value:
...
\begin{parameters}
  \param{quantile\_level}{Where the threshold on the RDI sits; the genes above it
...
\begin{expectedresults}
  With the default \code{quantile\_level=0.95} the flagged fraction is essentially
  ...
  $1-\optn{quantile\_level}$ means genes are reaching the test that should not:
  check for families with a zero centroid first.
\end{expectedresults}

\begin{pitfall}[Pitfall: reading the quantile level as a percentage]
  \optn{quantile\_level} is a fraction in $[0,1]$. Passing \code{95} does not ask
...
\begin{pitfall}[Pitfall: treating the tail probability as a p-value]
  The \code{tail\_probability} output states how extreme a gene's distance is
  ...
  \code{tail\_probability} of 0.001 is not a $p$ of 0.001 and must not be corrected for
...
\begin{interpretation}
  ...
  error rate. At \mbox{\code{quantile\_level=0.95}} you get the top 5\% by construction,
