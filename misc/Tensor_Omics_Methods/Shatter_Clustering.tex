...
\subsection{API Design Specifications}
The API should utilize interfaces (in Fortran, they define functions, not Types/Classes):
\begin{itemize}
    \item \textbf{CC label for a set of vectors:} Returns a $\mathbb{R}^k$.
    \item \textbf{Labeling Volume:} Can be configured as a sphere, a cone, etc.
    \item \textbf{Normalization:} For example, calculating the mean of a cluster's CC labels (Input Matrix $\rightarrow$ Output Matrix).
\end{itemize}