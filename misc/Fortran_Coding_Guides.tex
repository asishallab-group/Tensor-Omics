...
Any module that crosses the C boundary and treats \texttt{c\_int} as \texttt{int32} or
\texttt{c\_double} as \texttt{real64} must \texttt{use f42\_safeguard}. It contains compile-time
assertions that those kind constants really are equivalent on the target platform:

\begin{lstlisting}[language=Fortran]
use f42_safeguard   ! guarantees c_int == int32, c_double == real64, c_double_complex == real64
\end{lstlisting}

The generated \texttt{bind(C)} wrappers carry this themselves, so in practice the rule bites only
where a module declares a \texttt{bind(C)} interface by hand.
