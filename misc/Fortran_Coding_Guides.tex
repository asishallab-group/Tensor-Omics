...
\texttt{\_impl} & Core implementation subroutine, in a \texttt{\_impl} module & \texttt{k\_means\_assign\_cluster\_impl} \\
...
\texttt{arg\_pos} must always match the argument's position in the public (non-\texttt{\_impl}) subroutine that callers actually invoke, never its position in whichever internal implementation happens to run the validation.
...
Remove \texttt{pure} only when truly necessary (e.g., allocation, I/O), and only in the generated validating/allocating wrapper layer -- never in the \texttt{\_impl} itself.
...
\textbf{Core (\texttt{\_impl}) subroutines must not allocate memory.} Allocation is performed exclusively in the generated allocating wrapper layer. This rule:
\begin{itemize}
...
\begin{itemize}
  \item \textbf{Prohibited} in any subroutine or function that has a C wrapper or is otherwise called from Python or R.
    C, Python \texttt{ctypes}, and R's C \texttt{.Call} layer cannot represent Fortran derived types; passing them across the language boundary is undefined behaviour.
...
Snippets for Fortran, Python, and R are \textbf{auto-generated} by the code generator (\texttt{python helper/generate\_code.py}, target \texttt{snippets}) straight from each \texttt{\_impl} procedure's own documentation, and written to \texttt{snippets/}. You should never edit the generated files by hand -- the one hand-written exception is \texttt{snippets/toxdev\_snippets.json}, development-convenience snippets unrelated to any particular procedure. See \textbf{Writing for the Code Generator} above for what to write to get a procedure a snippet, and \texttt{helper/codegen/README.md} for the generator itself.
...
\begin{itemize}[label=$\square$]
  ...
  \item \texttt{\#include <src/macros.h>} is present if any \texttt{M\_*} macro is used.
  \item All \texttt{real} variables use \texttt{real(real64)}; all constants carry \texttt{\_real64}.
  \item All \texttt{logical} that occupy storage use \texttt{logical(c\_bool)}.
  \item Every argument has \texttt{intent(in|out|inout)}.
  \item The \texttt{\_impl} is declared \texttt{pure}, never validates input, and never allocates.
  \item Every constraint on an \texttt{\_impl} argument is a \texttt{DM\_*} macro in its \texttt{!!} comment, not prose alone.
  \item \texttt{do concurrent} is used for independent loops.
  \item No \texttt{GOTO} anywhere.
  \item Any module naming a C kind (\texttt{c\_bool}, \texttt{c\_int}, ...) has \texttt{use f42\_safeguard}.
  \item No derived types in any procedure the generator wraps.
  \item Reusable helpers are added to the \texttt{f42\_utils} family (\texttt{f42\_math\_impl}, \texttt{f42\_sort\_impl}, etc.) rather than duplicated.
  \item FORD comments (\texttt{!>}, \texttt{!|}, \texttt{!!}) are present for all public procedures -- they become the generated C/Python/R documentation verbatim.
  \item The code generator was run (\texttt{./build.sh}, or \texttt{helper/generate\_code.py} directly) and the regenerated files under \texttt{src/generated/}, \texttt{python/tensor\_omics/}, \texttt{r/tensor\_omics/} and \texttt{snippets/} are committed alongside the change.
