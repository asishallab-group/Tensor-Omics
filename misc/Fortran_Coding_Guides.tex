...
\begin{lstlisting}[language=Fortran]
! Preferred: explicit concurrency annotation
do concurrent (i_dim = 1:n_dims) shared(vector, vector_norm)
  vector(i_dim) = vector(i_dim) / vector_norm
end do

! With reduction (summing into a scalar is safe)
squares_sum = 0.0_real64
do concurrent (i_element = 1:n_elements) &
    shared(vec, mean_val) reduce(+:squares_sum)
...
\begin{itemize}
  ...
  \item \texttt{!!} — Continues a description on the next line (trail comment after a declaration).
  \item \texttt{!|} — Documents an argument; place this comment \textbf{on the line immediately after} the argument declaration, not before it.
\end{itemize}
...
% =============================================================================
\section{Getting Help: The Help Channel}
% =============================================================================

The help channel is the right place for \textbf{any question} — not just programming. Whether it is about scientific context, workflow setup, naming decisions, tooling, or general project conventions, there will always be someone available to help. Do not hesitate to ask.
