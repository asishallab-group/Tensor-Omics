...
\subsection{Every crash or error becomes a regression test}

Whenever a build, a test run, an experiment script, or manual exploration crashes or raises an error nobody expected, that failure is a missing unit test, not merely a bug to patch and move past.

\noindent \textbf{The sequence is always:} (1) write a Fortran test in \texttt{test/} that reproduces the failure \emph{condition}, (2) confirm it fails against the current code, (3) fix the underlying code, (4) confirm the same test now passes, (5) mirror the test to \texttt{python/test/} and \texttt{r/tests} per the usual convention -- unless the failure is genuinely unreachable from Python or R (for example, it depended on a raw Fortran call omitting an optional argument, something neither binding ever does, since both always resolve defaults explicitly before crossing the ABI boundary), in which case say so in a comment at the mirror site instead of forcing an artificial test.

\noindent \textbf{Never write a test that re-triggers an actual crash to prove it is fixed.} If the failure was a genuine crash -- a segfault, corrupted memory, undefined behavior -- fix the code \emph{first}, so the failure mode becomes a clean, deterministic result (a correct value, or a proper \texttt{ierr}), and only then write the regression test, asserting that deterministic result. A test that sometimes segfaults is worse than no test at all: it can take the whole test binary down with it, silently masking every test that would have run after it in the same suite, and it may behave differently across platforms, builds, and compiler flags.

\noindent This is not optional test hygiene -- an incident that produced a crash or a wrong answer once, with no test guarding against a recurrence, will eventually recur silently.
...
\subsubsection{Workaround: Binding Overloading}

To emulate default arguments, use multiple procedures with an binding:
...
\begin{itemize}
  ...
  \item Use binding overloading to simulate default values if needed for cleaner calling code.
  \item Do not use optional arguments in \texttt{elemental} procedures (not supported).
  \item Document the behavior and defaults of optional arguments clearly in the procedure's binding documentation.
\end{itemize}
...
\begin{itemize}
  ...
  \item Keep documentation up to date when the binding changes.
...
\section{Fortran to other programming languages Binding}

The Tensor Omics project exposes Fortran functionality to Python and R through a unified architecture that relies exclusively on C bindings. This approach ensures efficiency, portability, and maintainability.

\subsection{Architecture Overview}

The binding follows a three-layer architecture:

\begin{enumerate}
  \item \textbf{Core Fortran modules:} Pure computational routines implementing the scientific algorithms
  \item \textbf{C wrappers (\_C suffix):} Fortran subroutines with \texttt{bind(C)} that adapt the core functions to C calling conventions
  \item \textbf{Language-specific layers:}
  \begin{itemize}
    \item \textbf{Python:} Functions in \texttt{tensoromics.py} that call C wrappers via \texttt{ctypes}
    \item \textbf{R:} Functions in the \texttt{r/} package that call C wrappers via R's C \texttt{.Call} binding
  \end{itemize}
\end{enumerate}

\subsection{C Wrapper Implementation Pattern}

All C wrappers follow a consistent pattern designed for safety, efficiency, and zero-copy interoperation:

\begin{verbatim}
! Core Fortran function in module
module tox_relative_axis_plane_tools
contains
  pure subroutine relative_axes_changes_from_shift_vector(vec, n_axes, &
                                                         contributions, ierr)
...
! C wrapper with bind(C)
pure subroutine relative_axes_changes_from_shift_vector_c(vec, n_axes, contributions, ierr) 
...
\subsection{Key Principles for C Wrappers}

\subsubsection{Use \texttt{iso\_c\_binding} types exclusively}

All C wrappers must use standard C-interoperable types:
\begin{itemize}
  \item \texttt{real(c\_double)} for \texttt{real(real64)}
  \item \texttt{integer(c\_int)} for \texttt{integer(int32)}
  \item \texttt{character(kind=c\_char)} for character data
\end{itemize}


\subsubsection{Array arguments: explicit dimensions}

Arrays must be declared with explicit dimensions matching the C memory layout. Do not use assumed-shape arrays \texttt{(:)} in C wrappers:
\begin{verbatim}
! Correct - explicit dimensions
real(c_double), dimension(n_samples, n_genes), intent(in) :: expression_vectors

! Incorrect - assumed shape not allowed in bind(C)
real(c_double), dimension(:,:), intent(in) :: expression_vectors
\end{verbatim}

\subsubsection{Always include error code output}

All C wrappers should accept an \texttt{ierr} parameter:
\begin{verbatim}
integer(c_int), intent(out) :: ierr
\end{verbatim}

Initialize it with \texttt{call set\_ok(ierr)} at the beginning of the wrapper.

\subsubsection{Type conversions for character data}

When Fortran string types are involved, perform explicit conversions:
\begin{verbatim}
...
\subsection{Memory Management and Zero-Copy Operation}

\textbf{Critical:} C wrappers operate \emph{directly} on the memory provided by the caller. No copies are made. This is essential for performance with large omics datasets.

The caller (Python or R) must:
\begin{itemize}
  \item Allocate arrays with correct dimensions
  \item Ensure Fortran column-major order (F-contiguous)
  \item Maintain memory validity throughout the call
\end{itemize}

\subsection{Python Binding via ctypes}

Python wrappers in \texttt{tensoromics\_functions.py} follow this pattern:
...
\subsection{R Binding via C \texttt{.Call}}

R wrappers in \texttt{tensoromics\_functions.cpp} follow this pattern:
...
/* Pure-C .Call shim (registered in init.c), called from R via
   .Call("relative_axes_expression_from_expression_vector_call", vec) */
SEXP relative_axes_expression_from_expression_vector_call(SEXP vec) {
    ...
    relative_axes_expression_from_expression_vector_c(REAL(vec),  /* zero-copy */
                                                      &n_axes,
                                                      REAL(contributions),
                                                      &ierr);

    SEXP out = PROTECT(Rf_allocVector(VECSXP, 2)); nprot++;
    SET_VECTOR_ELT(out, 0, contributions);
    SET_VECTOR_ELT(out, 1, Rf_ScalarInteger(ierr));
    SEXP nms = PROTECT(Rf_allocVector(STRSXP, 2)); nprot++;
    SET_STRING_ELT(nms, 0, Rf_mkChar("contributions"));
...
Key points:
\begin{itemize}
  \item All functions must be documented with Roxygen comments for automatic documentation generation.
  \item All data validation is on the R side, not the C marshalling layer
  \item In python all outputs must be converted to read-only to prevent accidental modification while Fortran is accessing them. 
  \item Errors are handled by checking the \texttt{ierr} code by using the tox\_errors functions. 
\end{itemize}

\subsection{When to Write Wrappers}

Only create C wrappers for functions that provide essential scientific functionality. Generic utilities (sorting, I/O helpers) should not be wrapped unless they are core to the Tensor Omics workflow.

\subsection{Development Workflow: Mandatory Wrappers and Documentation}

To maintain API parity across Python and R, the following steps are \textbf{mandatory} when implementing new functionality:

\begin{enumerate}
    \item \textbf{Core Implementation:} Write the pure Fortran function in the appropriate module
    \item \textbf{C Wrapper:} Create a \texttt{\_C} wrapper with \texttt{bind(C)}
    \item \textbf{Python and R bindings:} Run the code generator (\texttt{helper/generate\_code.py}); it emits the Python \texttt{ctypes} module (\texttt{python/tensor\_omics/}) and the R C \texttt{.Call} shim under an R wrapper (\texttt{r/tensor\_omics/}) from the documented Fortran source --- these are not hand-written
    \item \textbf{Documentation:} Update user manual with:
    \begin{itemize}
        \item Description of functionality
        \item Parameter explanations
        \item Usage examples for Fortran, Python and R
        \item Memory considerations (if arrays are modified in-place)
    \end{itemize}
    \item \textbf{Tests:} Write unit tests in Fortran, and test callings in Python/R
\end{enumerate}

\textbf{Memory Safety Warning:} When using direct array access from Python or R, users must ensure that arrays are not modified by other libraries (e.g., matplotlib, numpy, ggplot2) while Fortran code is accessing them. This can lead to undefined behavior or memory corruption.


\subsection{Safe and Efficient Binding Design for \texttt{bind(C)} in Tensor Omics}
...
Optional arguments are not allowed in R and C wrappers because \texttt{bind(C)} is not compatible with them. If optional arguments are needed, they should only be used in core subroutines that are called directly from Fortran. 
...
\subsection{Argument Passing Conventions in C Wrappers}

All arguments in C wrappers are passed \textbf{by reference}, including scalar values. This is the standard Fortran calling convention and ensures zero-copy operation.

\subsubsection{Example of by-reference passing}

\begin{verbatim}
! Even scalars are passed by reference
integer(c_int), intent(in), value :: n_genes     ! INCORRECT
integer(c_int), intent(in), target :: n_genes     ! CORRECT - by reference
...
\subsection{Null Pointer Validation}

All C wrappers \textbf{must validate} that pointers are not null before use. This is done using preprocessor macros defined in \texttt{src/macros.h}.

\subsubsection{Required macros for null validation}

\begin{verbatim}
#include "macros.h"

subroutine my_function_c(..., ierr) bind(C, name="my_function_c")
  use, intrinsic :: iso_c_binding, only: c_int, c_double
  M_USE_NULL_VALIDATION
  implicit none
  
  integer(c_int), intent(in), target :: n_genes
  real(c_double), dimension(n_genes), intent(in), target :: data
  integer(c_int), intent(out), target :: ierr
  
  ! ALWAYS check ierr first
  M_CHECK_IERR_NON_NULL
  
  ! Then check all other arguments
  M_CHECK_NON_NULL(n_genes)
  M_CHECK_NON_NULL(data)
...
\textbf{Key macros:}
\begin{itemize}
  \item \texttt{M\_USE\_NULL\_VALIDATION}: Include at top of wrapper to import required modules
  \item \texttt{M\_CHECK\_IERR\_NON\_NULL}: Always first check - validates \texttt{ierr} pointer
  \item \texttt{M\_CHECK\_NON\_NULL(arg)}: Validates each argument pointer, sets \texttt{ierr = ERR\_POINTER\_NULL} if null
\end{itemize}

\textbf{Note:} All arguments in C wrappers must have the \texttt{target} attribute to allow pointer validation with \texttt{c\_loc()}.

\subsection{Utility Macros for C Wrappers}

The \texttt{src/macros.h} file provides several useful preprocessor macros to simplify common patterns in C wrappers.

\subsubsection{Default value handling}

Use \texttt{M\_DEFAULT\_VAL} to handle optional parameters with default values:

\begin{verbatim}
pure subroutine my_function_c(..., max_angle, gain_gamma, ierr) bind(C)
  real(c_double), intent(in), target :: max_angle
  real(c_double), intent(in), target :: gain_gamma
  
  real(real64) :: local_max_angle, local_gain
  
  ! Set default if value matches sentinel (e.g., NaN or -1)
  M_DEFAULT_VAL(max_angle, local_max_angle, PI)
  M_DEFAULT_VAL(gain_gamma, local_gain, 0.1_real64)
  
  ! Use local_max_angle and local_gain in computation
end subroutine
\end{verbatim}

\subsubsection{Safe allocation with error checking}

Use \texttt{M\_ALLOCATE} to allocate with automatic error handling:
...
Macros for IEEE special values:

\begin{verbatim}
real(real64) :: nan_value, pos_inf, neg_inf

nan_value = M_NAN          ! IEEE quiet NaN
pos_inf = M_POS_INF        ! Positive infinity
neg_inf = M_NEG_INF        ! Negative infinity
\end{verbatim}

\subsubsection{Complete macro reference}

\begin{verbatim}
! From src/macros.h:

M_USE_NULL_VALIDATION          ! Import modules for null checking
M_CHECK_IERR_NON_NULL          ! Validate ierr pointer
M_CHECK_NON_NULL(ARG)          ! Validate argument pointer
M_DEFAULT_VAL(OPT, VAR, DEF)   ! Set default value
M_ALLOCATE(VAR_DECL)           ! Allocate with error check
M_NAN                          ! IEEE quiet NaN
M_POS_INF                      ! Positive infinity
M_NEG_INF                      ! Negative infinity
\end{verbatim}

\subsection{Error Handling with tox\_errors}

Tensor Omics uses a standardized error handling system defined in \texttt{src/tox\_errors.F90}. All functions must follow this convention.

\subsubsection{Error codes}

The \texttt{tox\_errors} module defines standard error codes:
...
subroutine my_function(..., ierr)
  ...
  ! 3. Check error before proceeding
...
\subsubsection{Validation helper functions}

The \texttt{tox\_errors} module provides validation helpers:

\begin{verbatim}
! Validate dimension is positive and non-zero
call validate_dimension_size(n_genes, ierr)

! Validate value is in range
call validate_in_range_int(value, ierr, min=0_int32, max=100_int32)
call validate_in_range_real(angle, ierr, min=0.0_real64, max=PI)

! Validate entire array
call validate_all_in_range_real(expression_vectors, n_total, ierr, &
                                min=-1.0_real64, max=1.0_real64)
\end{verbatim}

\subsubsection{Error propagation}

Errors should propagate up the call stack without modification:

\begin{verbatim}
subroutine caller(ierr)
  call worker_a(ierr)
  if (is_err(ierr)) return  ! Propagate error immediately
...
Use \texttt{set\_err\_once()} to set error only if not already set:

\begin{verbatim}
call set_err_once(ierr, ERR_INVALID_INPUT)  ! Won't overwrite existing error
\end{verbatim}
...
\textbf{Meaning:} \texttt{a(:)} is an assumed-shape array. The compiler receives size and bounds information at runtime. Requires an explicit binding (typically via a module or \texttt{binding} block).
...
\textbf{Meaning:} Each colon represents a dimension of unknown size. Like 1D assumed-shape arrays, this requires an explicit binding and passes shape info at runtime.
...
\noindent For all assumed-shape forms (\texttt{(:)}, \texttt{(:,:)}), ensure the subroutine has an explicit binding or is in a module.
...
\section{Critical Practices in TOX Binding and Control Flow}

This section consolidates key implementation standards for TOX and F42 compliance: prohibition of \texttt{GOTO} and manual memory cleanup.
...
All control flow and memory management in TOX must follow F42 principles:
\begin{itemize}
  ...
  \item Use C wrappers with \texttt{bind(C)} for binding with Python and R.
  \item Ensure proper error handling via \texttt{ierr} parameter in all API functions.
\end{itemize}
...
\subsection{Scope}
This specification defines the design principles, coding conventions, and binding structure for the Tensor Omics Scientific Kernel (SK) and its associated API helper layers. The aim is to ensure performance, reproducibility, portability, and clarity of responsibility between computational components.
...
\begin{enumerate}
  ...
  \item Rich querying is provided via a GraphQL binding. We might implement a
    `jq` binding on top of that, too.
\end{enumerate}
