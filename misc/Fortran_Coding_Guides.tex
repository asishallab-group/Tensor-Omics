...
The interface follows a three-layer architecture:
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item \textbf{R:} Functions in the \texttt{r/} package that call C wrappers via R's C \texttt{.Call} interface
  \end{itemize}
\end{enumerate}
...
\subsection{Python Interface via ctypes}
...
\subsection{R Interface via C \texttt{.Call}}
...
\begin{enumerate}
    ...
    \item \textbf{Python and R interfaces:} Run the code generator (\texttt{helper/generate\_code.py}); it emits the Python \texttt{ctypes} module (\texttt{python/tensor\_omics/}) and the R C \texttt{.Call} shim under an R wrapper (\texttt{r/tensor\_omics/}) from the documented Fortran source --- these are not hand-written
...
\begin{itemize}
  ...
  \item Use C wrappers with \texttt{bind(C)} for interfacing with Python and R.
