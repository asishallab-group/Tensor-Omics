...
\textbf{Characters are not the same case.} There is no matching rule to use
\texttt{character(kind=c\_char)} everywhere: an implementation declares an ordinary
\texttt{character(len=*)}, and \texttt{c\_char} appears only where a buffer really is a C
buffer, which in hand-written code is the libzip interface and the view helper in
\texttt{tox\_conversions}. What an implementation \emph{does} need to know is that a character
argument arrives blank-padded to the buffer width rather than trimmed --- see
\texttt{codegen\_guide.md}.
