...
If you already have \texttt{gfortran} $\geq$ 15 installed, compile directly with \texttt{build.sh}. It compiles all files in \texttt{src/}, places compiled objects under \texttt{build/<compiler>/}, and creates a symbolic link to the resulting shared library (\texttt{libtensor\_omics.so}) so that Python and R always find the same path.
...
\paragraph{Bit masks.} A mask that an implementation builds and combines itself -- the members of an ensemble, the candidates of a search, one mask per seed or subset -- is held as a bit mask from \texttt{f42\_bit\_masks\_impl} instead: 32 flags per \texttt{integer(int32)} word, an eighth of the memory of \texttt{logical(c\_bool)}, and whole-mask operations such as \texttt{bit\_mask\_and} or the fused \texttt{bit\_mask\_and\_count} handle 32 flags at once. A mask the user passes in stays a \texttt{logical(c\_bool)} array, because that is what numpy and R build; convert at the edge with \texttt{bit\_mask\_from\_logical\_impl}. The words are plain arrays, not a derived type -- \texttt{misc/bench/bit\_masks/} records why -- so a column \texttt{bit\_masks(:, i\_mask)} of an \texttt{(n\_words, n\_masks)} array is a mask of its own and passes to any procedure without a copy. In hot loops, work on whole words or step from one set flag to the next with \texttt{bit\_mask\_next}; where a loop must test single flags, the \texttt{M\_BIT\_MASK\_TEST} macro does it inline, since a procedure call per flag costs more than the flag. A parallel loop writes a mask word by word, never flag by flag: two iterations setting flags in the same word race.
...
\texttt{M\_EXPORT\_C} & Exports a hand-written procedure; see \texttt{codegen\_guide.md}, Part II \\
...
\texttt{M\_CHECK\_IO\_ERR(CODE)} & After I/O: on a non-zero \texttt{ierr}, sets \texttt{CODE}, closes \texttt{unit}, returns \\
...
\texttt{M\_GENE\_TO\_FAM\_SENTINEL} & The \texttt{gene\_to\_fam} value of a gene in no family \\
\texttt{M\_GENE\_TO\_FAM\_DOC(ARG)} & The standard doc sentence for a \texttt{gene\_to\_fam} argument \\
\bottomrule
\end{tabular}

The \texttt{M\_CHECK\_*} null checks in \texttt{macros.h} are not listed: the generator emits them
into the C wrappers, and no hand-written code uses them.
...
at every single declaration naming it. For \texttt{c\_bool}, which every logical in the framework
now uses, that is an error in nearly every file --- none of which did anything wrong --- instead of
one here naming the kind that is missing.
