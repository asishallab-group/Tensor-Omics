...
# build/libtensor_omics.so, so the compute time is identical and cancels out —
...
so_path    <- file.path(build_dir, "libtensor_omics.so")
...
# Just -ltensor_omics: the Fortran runtime (Intel iomp5/ifcore) is pulled in via
# the .so's own NEEDED entries, so no -lgfortran here. Path is quoted: the repo
# path contains a space.
pkg_libs <- sprintf("-L%s -Wl,-rpath,%s -ltensor_omics",
...
# pure-C shim: same R CMD SHLIB compile, but a .c file — no C++ toolchain at all.
cat("Compiling pure-C shim (R CMD SHLIB) ...\n")
c_so <- file.path(script_dir, "loess_c.so")
Sys.setenv(PKG_LIBS = pkg_libs)
c_shlib_cmd <- sprintf("cd %s && %s CMD SHLIB loess_c.c -o loess_c.so",
                       shQuote(script_dir), shQuote(rbin))
if (system(c_shlib_cmd) != 0) stop("pure-C R CMD SHLIB compile failed")
dyn.load(c_so)
stopifnot(is.loaded("loess_smooth_2d_ccall"))
loess_smooth_2d_ccall <- function(x_ref, y_ref, indices_used, x_query, ks, kc)
    .Call("loess_smooth_2d_ccall", x_ref, y_ref, indices_used, x_query, ks, kc)
...
call_c     <- function(x_ref, y_ref, indices_used, x_query, ks, kc)
    loess_smooth_2d_ccall(x_ref, y_ref, indices_used, x_query, ks, kc)$y_out

variants <- list(rcpp = call_rcpp, cpp11 = call_cpp11, c = call_c, dotC = call_dotC)
...
stopifnot(isTRUE(all.equal(ys$rcpp, ys$cpp11)),
          isTRUE(all.equal(ys$rcpp, ys$c)),
          isTRUE(all.equal(ys$rcpp, ys$dotC)))
cat("  OK — all four variants agree on y_out\n")
...
tbl <- do.call(rbind, lapply(rows, function(r)
    data.frame(n_total = r$n_total, payload_MB = round(r$payload_MB),
               ...
               c_ms = round(r$c, 3), dotC_ms = round(r$dotC, 3))))
...
md <- c(
    ...
    "`build/libtensor_omics.so`. The routine is driven with large read-only",
    ...
    "|--------:|-----------:|--------:|---------:|-----:|------:|",
    apply(tbl, 1, function(r) sprintf("| %s | %s | %s | %s | %s | %s |",
        fmt_int(as.integer(r["n_total"])), fmt_int(as.integer(r["payload_MB"])),
        r["rcpp_ms"], r["cpp11_ms"], r["c_ms"], r["dotC_ms"])),
    "",
    ...
    "",
    sprintf(paste0(
        "- **Zero-copy holds for all three `.Call` shims — Rcpp, cpp11, and pure C ",
        "are at parity.** All stay flat at ~%.2f ms/call across a %dx growth in ",
        "input size — that floor is R call dispatch plus the trivial ",
        "O(n_target*n_used) compute, and it does *not* grow with the reference ",
        "arrays. The differences between the three are below `proc.time()` ",
        "resolution at these rep counts, i.e. noise, not signal."),
        mean(c(tbl$rcpp_ms, tbl$cpp11_ms, tbl$c_ms)),
    ...
    sprintf(paste0(
        ...
        round(tbl$dotC_ms[which.max(tbl$n_total)] /
              mean(c(tbl$rcpp_ms[which.max(tbl$n_total)],
                     ...
                     tbl$c_ms[which.max(tbl$n_total)])))),
