...
\begin{enumerate}
    ...
    \textbf{Output:}
    \begin{itemize}
        \item \texttt{np.array}: Permutation indices (0-based Python indices)
    \end{itemize}
...
\end{lstlisting}
    ...
    \begin{itemize}
        ...
        \item \texttt{indices numpy.ndarray}: BST index array from build\_bst\_index (0-based)
        \item \texttt{lower\_bound float}: Lower bound of range (inclusive)
        \item \texttt{upper\_bound float}: Upper bound of range (inclusive)
    \end{itemize}
    \textbf{Output:}
    \begin{itemize}
        \item \texttt{tuple}: (matching\_indices, count) where indices are 0-based
    \end{itemize}
\begin{lstlisting}[language=Python]
matching_indices, count = bst_range_query(values, indices, lower_bound, upper_bound)
\end{lstlisting}
