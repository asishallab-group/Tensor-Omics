...
The Python module \texttt{tensoromics\_functions} provides six main functions for array handling:
...
\begin{itemize}
    \item \texttt{array} (input): NumPy array to be serialized \\
          \textbf{Type:} \texttt{numpy.ndarray} with dtype \texttt{np.int32} or \texttt{np.float64}, for characters: \texttt{U5} \\
...
\begin{itemize}
    \item \texttt{array} (input): R array, matrix, or vector to be serialized \\
          \textbf{Type:} \texttt{integer} or \texttt{numeric} (double precision), or characters \\
...
\begin{itemize}
    ...
    \item Supports standard R data types (integer, numeric, character)
