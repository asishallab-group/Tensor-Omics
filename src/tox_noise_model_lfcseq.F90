...
contains
    ...
    subroutine compute_noise_pvalue_pipeline_helper( &
        ...
        ! This loop MUST stay sequential: the `own` within-neighbourhood null
        ! (compute_pvalue_lfcseq_helper) draws from the global RNG stream seeded once
        ! by init_random(42). Parallelising the gene loop would make the draws
        ! order-dependent and break reproducibility -- use a per-gene counter-based
        ! RNG (seeded by gene index) before ever doing so.
        do i_gene = 1, n_genes
            ...
            ! that passed the gates (all three compute-flags can be 0).
