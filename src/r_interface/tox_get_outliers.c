...
void detect_outliers_c(const int*, const int*, const double*, const int*, double*, int*, int*, int*, unsigned char*, double*, double*, int*, double*, int*, const double*);
...
SEXP compute_family_scaling_call(SEXP n_families, SEXP distances, SEXP gene_to_fam) {
    int nprot = 0;
    // derived from the inputs, not asked of the caller
    int n_genes = (int) Rf_length(distances);
