...
SEXP ensemble_identification_merged_call(SEXP vectors, SEXP kd_indices, SEXP dimension_order, SEXP seed_selection_mask, SEXP k_min, SEXP alpha_max, SEXP d_max, SEXP G_max, SEXP f_max, SEXP a, SEXP o) {
    int nprot = 0;
    // derived from the inputs, not asked of the caller
    int n_dimensions = INTEGER(Rf_getAttrib(vectors, R_DimSymbol))[0];
    int n_vectors = INTEGER(Rf_getAttrib(vectors, R_DimSymbol))[1];
