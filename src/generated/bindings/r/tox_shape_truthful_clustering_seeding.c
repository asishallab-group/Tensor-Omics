...
SEXP density_labels_call(SEXP vectors, SEXP kd_indices, SEXP dimension_order, SEXP k_min, SEXP bandwidth_percentile) {
    int nprot = 0;
...
SEXP seeds_call(SEXP vectors, SEXP kd_indices, SEXP dimension_order, SEXP k_min, SEXP bandwidth_percentile, SEXP exclusion_radius_percentile) {
    int nprot = 0;
    // derived from the inputs, not asked of the caller
    int n_dimensions = INTEGER(Rf_getAttrib(vectors, R_DimSymbol))[0];
    int n_vectors = INTEGER(Rf_getAttrib(vectors, R_DimSymbol))[1];
