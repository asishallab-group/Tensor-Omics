...
module tox_shape_truthful_clustering_reconciliation
    use tox_shape_truthful_clustering_reconciliation_kernel, only: MODE_MERGE_ANY, MODE_MERGE_JSI, MODE_REPORT, ensemble_reconciliation_kernel
...
contains
    ...
    subroutine ensemble_reconciliation(&
        ...
        integer(int32), intent(out) :: n_super_ensembles
            ...
            !! filled
        real(real64), dimension(max_group_size-1, n_ensembles*(n_ensembles-1)), intent(out) :: super_ensembles_JSI
        ...
        call validate_in_range_real(min_jsi, ierr, arg_pos=5_int32, min=0.0_real64, max=1.0_real64)
        call validate_in_range_int(max_group_size, ierr, arg_pos=7_int32, min=2_int32, max=n_ensembles)
        if (present(mode)) then; if (mode /= MODE_REPORT .and. mode /= MODE_MERGE_JSI .and. mode /= MODE_MERGE_ANY) call set_err_once(ierr, ERR_INVALID_INPUT, arg_pos=4_int32); end if
