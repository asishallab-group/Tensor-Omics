...
!> summary: Wrappers for [[tox_get_outliers_kernel(module)]]
...
module tox_get_outliers
    ...
    use tox_loess_kernel, only: MODE_PLAIN, MODE_ROBUST, tox_loess_required_workspace
    ...
    public :: compute_family_scaling_alloc
    public :: compute_rdi
    public :: compute_rdi_alloc
    public :: identify_outliers
    public :: detect_outliers
    public :: detect_outliers_alloc

contains

    !> summary: Validates its inputs, then calls [[tox_get_outliers_kernel(module):compute_family_scaling_kernel]].
    ...
    !| Uses LOESS on the median/stddev of intra-family distances for scaling, regardless of orthologs.
    subroutine compute_family_scaling_alloc(&
            n_genes,&
            n_families,&
            distances,&
            gene_to_fam,&
    ...
    !> summary: Allocates its work arrays, then calls [[tox_get_outliers_kernel(module):compute_rdi_kernel]].
    !| RDI = Euclidean distance / family scaling factor
    subroutine compute_rdi_alloc(&
            n_genes,&
            distances,&
            gene_to_fam,&
            dscale,&
    ...
    end subroutine compute_rdi_alloc

    !> summary: Validates its inputs, then calls [[tox_get_outliers_kernel(module):identify_outliers_kernel]].
    ...
    !> summary: Validates its inputs, then calls [[tox_get_outliers_kernel(module):detect_outliers_kernel]].
    ...
    !| [[tox_get_outliers_kernel(module):compute_rdi_kernel(subroutine)]], then flags outliers via
    !| [[tox_get_outliers_kernel(module):identify_outliers_kernel(subroutine)]].
    subroutine detect_outliers_alloc(&
            n_genes,&
            n_families,&
            distances,&
            gene_to_fam,&
