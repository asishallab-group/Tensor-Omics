...
!> summary: Wrappers for [[tox_data_integration_per_family_kernel(module)]]
!| Generated from the kernel; do not edit -- regenerate instead.
module tox_data_integration_per_family
    use tox_data_integration_per_family_kernel, only: fjct_compute_contribution_scores_kernel, fjct_compute_jsd_kernel, fjct_compute_masked_jsd_kernel
    ...
    public :: fjct_compute_jsd_alloc
    public :: fjct_compute_masked_jsd
    public :: fjct_compute_masked_jsd_alloc
    public :: fjct_compute_contribution_scores

contains

    !> summary: Validates its inputs, then calls [[tox_data_integration_per_family_kernel(module):fjct_compute_jsd_kernel]].
    ...
    !> summary: Allocates its work arrays, then calls [[tox_data_integration_per_family_kernel(module):fjct_compute_jsd_kernel]].
    !| Reuses the same conditioning-on-mean-expression pipeline as the global gJCT, but restricts
    !| the residual samples to the genes belonging to the family `family_idx`.
    subroutine fjct_compute_jsd_alloc(&
            family_idx,&
            gene_to_family_S1,&
            gene_to_family_S2,&
            n_genes_S1,&
    ...
    !> summary: Validates its inputs, then calls [[tox_data_integration_per_family_kernel(module):fjct_compute_masked_jsd_kernel]].
    ...
    !> summary: Allocates its work arrays, then calls [[tox_data_integration_per_family_kernel(module):fjct_compute_masked_jsd_kernel]].
    !| Reuses the same conditioning-on-mean-expression pipeline as the global gJCT, but restricts the
    !| residual samples to the neighbors selected by `neighbor_mask_S1`/`neighbor_mask_S2`. Typically
    !| those are all neighbors belonging to one gene family, which is what `fjct_compute_jsd` builds
    !| the masks for from a family index.
    subroutine fjct_compute_masked_jsd_alloc(&
            neighborhood_residuals_S1,&
            neighborhood_residuals_S2,&
            n_reps_S1,&
            n_reps_S2,&
    ...
    end subroutine fjct_compute_masked_jsd_alloc

    !> summary: Validates its inputs, then calls [[tox_data_integration_per_family_kernel(module):fjct_compute_contribution_scores_kernel]].
    !| Combines
    !|
