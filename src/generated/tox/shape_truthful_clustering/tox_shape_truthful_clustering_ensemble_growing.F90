...
!|
!| Generated from [[tox_shape_truthful_clustering_ensemble_growing_impl(module)]]; do not edit -- regenerate instead.
module tox_shape_truthful_clustering_ensemble_growing
    use f42_safeguard
    use tox_shape_truthful_clustering_ensemble_growing_impl, only: calc_ensemble_growth_radius_impl, grow_ensemble_impl
    ...
    public :: calc_ensemble_growth_radius_expert
    public :: grow_ensemble
    public :: grow_ensemble_expert

contains

    !> summary: Validates its inputs, prepares what [[tox_shape_truthful_clustering_ensemble_growing_impl(module):calc_ensemble_growth_radius_impl]] needs, then calls it. The entry point to reach for first; see [[tox_shape_truthful_clustering_ensemble_growing(module):calc_ensemble_growth_radius_expert]] to prepare it yourself.
    ...
    pure subroutine calc_ensemble_growth_radius(&
            vectors,&
            n_dimensions,&
            n_vectors,&
            kd_indices,&
        ...
        call calc_ensemble_growth_radius_impl(&
            vectors = vectors,&
    ...
    !> summary: Validates its inputs, then calls [[tox_shape_truthful_clustering_ensemble_growing_impl(module):calc_ensemble_growth_radius_impl]] with what you supply. The expert entry point: it allocates nothing and prepares nothing; [[tox_shape_truthful_clustering_ensemble_growing(module):calc_ensemble_growth_radius]] does both.
    !| Matches LoManLe's `local_scale_i` exactly at the default `radius_percentile=50.0` (the
    !| median) -- a per-seed, locally adaptive radius rather than a single dataset-wide one
    !| (see `misc/STC_for_LoManLe.md` section 2.2). Work arrays are sized for the worst case
    !| (`k_min = n_vectors - 1`) and sliced internally, since `k_min`'s resolved value is only
    ...
    !| shrink its exclusion radius independently of this kernel's own default, without
    !| touching the actual growth-radius computation any other caller relies on.
    pure subroutine calc_ensemble_growth_radius_expert(&
            vectors,&
            n_dimensions,&
            n_vectors,&
            kd_indices,&
    ...
    end subroutine calc_ensemble_growth_radius_expert

    !> summary: Validates its inputs, prepares what [[tox_shape_truthful_clustering_ensemble_growing_impl(module):grow_ensemble_impl]] needs, then calls it. The entry point to reach for first; see [[tox_shape_truthful_clustering_ensemble_growing(module):grow_ensemble_expert]] to prepare it yourself.
    ...
    pure subroutine grow_ensemble(&
        ...
        integer(int32), dimension(n_dimensions), intent(in) :: dimension_order
            ...
            !! The minimum valid value is `1_int32`.
            !! The maximum valid value is `n_dimensions`.
        logical(c_bool), dimension(n_vectors), intent(in) :: is_member_mask
        ...
        real(real64), intent(in) :: growth_radius
            ...
            !! The minimum valid value is `0.0_real64`.
        logical(c_bool), dimension(n_vectors), intent(out) :: is_member_mask_next
            !! Grown ensemble membership (superset of `is_member_mask`)
        integer(int32), intent(out) :: ierr
            !! Error code; zero on success, non-zero on failure.
        integer(int32), dimension(:, :), allocatable :: tmp_range_stack
        logical(c_bool), dimension(:), allocatable :: tmp_member_mask_buf
    ...
    !> summary: Validates its inputs, then calls [[tox_shape_truthful_clustering_ensemble_growing_impl(module):grow_ensemble_impl]] with what you supply. The expert entry point: it allocates nothing and prepares nothing; [[tox_shape_truthful_clustering_ensemble_growing(module):grow_ensemble]] does both.
    !| $\mathcal{C}_{\mathcal{E}_{t+1}} = \{x_k \mid \|x_k-x_i\|\le r_{\mathcal{E}} \;\exists\,
    !| x_i\in\mathcal{E}\}$. Deliberately a plain sequential loop, not parallelized: this runs
    !| once per growth iteration per ensemble, and outer-level parallelism across
    !| ensembles/seeds (in `ensemble_identification`) is the right place for that -- a single
    ...
    !| `is_member_mask` (an empty ensemble) is a well-defined degenerate case: there is nothing
    !| to grow from, so the result is all-.false. too, not an error.
    pure subroutine grow_ensemble_expert(&
            vectors,&
            n_dimensions,&
            n_vectors,&
            kd_indices,&
        ...
        logical(c_bool), dimension(n_vectors), intent(out) :: tmp_member_mask_buf
    ...
    end subroutine grow_ensemble_expert
