...
!|
!| Generated from [[tox_shape_truthful_clustering_parameter_estimation_impl(module)]]; do not edit -- regenerate instead.
module tox_shape_truthful_clustering_parameter_estimation
    use f42_safeguard
    use tox_shape_truthful_clustering_parameter_estimation_impl, only: estimate_stc_parameters_impl, grow_estimator_anchor_clouds_impl, sample_estimator_anchors_impl, tox_stc_estimate_parameters_svd_workspace
    ...
    public :: sample_estimator_anchors_expert
    public :: grow_estimator_anchor_clouds
    public :: estimate_stc_parameters
    public :: estimate_stc_parameters_expert

contains

    !> summary: Validates its inputs, prepares what [[tox_shape_truthful_clustering_parameter_estimation_impl(module):sample_estimator_anchors_impl]] needs, then calls it. The entry point to reach for first; see [[tox_shape_truthful_clustering_parameter_estimation(module):sample_estimator_anchors_expert]] to prepare it yourself.
    ...
    pure subroutine sample_estimator_anchors(&
            density_labels,&
            n_vectors,&
            n_anchors,&
            anchor_indices,&
        ...
        call sample_estimator_anchors_impl(&
    ...
    !> summary: Validates its inputs, then calls [[tox_shape_truthful_clustering_parameter_estimation_impl(module):sample_estimator_anchors_impl]] with what you supply. The expert entry point: it allocates nothing and prepares nothing; [[tox_shape_truthful_clustering_parameter_estimation(module):sample_estimator_anchors]] does both.
    !| Nearest-rank selection (not `calc_percentile_impl`'s own linear interpolation): these
    !| must be genuine point indices, not interpolated values. Percentiles are
    !| $100/n_{\text{anchors}}, 200/n_{\text{anchors}}, \ldots, 100$ -- e.g. $n_{\text{anchors}}=5$
    !| gives 20/40/60/80/100%ile. Duplicate anchor indices are possible (not deduplicated) when
    !| `n_anchors` is close to `n_vectors`; harmless -- a duplicated anchor's cloud just grows
    !| redundantly, one estimator-anchor "slot" among several effectively wasted, not incorrect.
    pure subroutine sample_estimator_anchors_expert(&
            density_labels,&
            n_vectors,&
            n_anchors,&
            tmp_sort_perm,&
    ...
    end subroutine sample_estimator_anchors_expert

    !> summary: Validates its inputs, then calls [[tox_shape_truthful_clustering_parameter_estimation_impl(module):grow_estimator_anchor_clouds_impl]].
    ...
    pure subroutine grow_estimator_anchor_clouds(&
            vectors,&
            n_dimensions,&
            n_vectors,&
            anchor_indices,&
        ...
        logical(c_bool), dimension(n_vectors, n_anchors), intent(out) :: cloud_masks
        ...
        call grow_estimator_anchor_clouds_impl(&
            vectors = vectors,&
    ...
    !> summary: Validates its inputs, prepares what [[tox_shape_truthful_clustering_parameter_estimation_impl(module):estimate_stc_parameters_impl]] needs, then calls it. The entry point to reach for first; see [[tox_shape_truthful_clustering_parameter_estimation(module):estimate_stc_parameters_expert]] to prepare it yourself.
    ...
    !| 5.14.
    pure subroutine estimate_stc_parameters(&
            vectors,&
            n_dimensions,&
            n_vectors,&
            kd_indices,&
        ...
        logical(c_bool), dimension(:, :), allocatable :: tmp_cloud_masks
        ...
        call estimate_stc_parameters_impl(&
            vectors = vectors,&
            n_dimensions = n_dimensions,&
            n_vectors = n_vectors,&
    ...
    !> summary: Validates its inputs, then calls [[tox_shape_truthful_clustering_parameter_estimation_impl(module):estimate_stc_parameters_impl]] with what you supply. The expert entry point: it allocates nothing and prepares nothing; [[tox_shape_truthful_clustering_parameter_estimation(module):estimate_stc_parameters]] does both.
    !| Orchestrates density_labels -> sample_estimator_anchors -> grow_estimator_anchor_clouds
    !| -> observable (once per EA) -> pairwise EA comparisons -> aggregation. See
    !| `misc/mod_STC.md`, SKG `estimate_stc_parameters`, for the full definition of every
    !| output and the reasoning behind each. EAs whose final cloud has fewer than 2 members
    ...
    !| n_vectors/n_anchors/seed_max_set_size in isolation) -- see `codegen_guide.md` section
    !| 5.14.
    pure subroutine estimate_stc_parameters_expert(&
            vectors,&
            n_dimensions,&
            n_vectors,&
            kd_indices,&
        ...
        integer(int32), intent(in) :: lwork_observable
            !! Size of tmp_work
            !! It is *VERY IMPORTANT* to compute this argument from the `lwork_observable` output produced by [[tox_shape_truthful_clustering_parameter_estimation_impl(module):tox_stc_estimate_parameters_svd_workspace]].
        integer(int32), intent(in) :: iwork_size
            !! Size of tmp_iwork
            !! It is *VERY IMPORTANT* to compute this argument from the `iwork_size` output produced by [[tox_shape_truthful_clustering_parameter_estimation_impl(module):tox_stc_estimate_parameters_svd_workspace]].
        integer(int32), intent(in) :: lwork_angle
            !! Size of tmp_angle_work
            !! It is *VERY IMPORTANT* to compute this argument from the `lwork_angle` output produced by [[tox_shape_truthful_clustering_parameter_estimation_impl(module):tox_stc_estimate_parameters_svd_workspace]].
        real(real64), dimension(n_dimensions, n_vectors), intent(in) :: vectors
            !! Input data matrix
        integer(int32), dimension(n_vectors), intent(in) :: kd_indices
            !! Pre-built k-d tree index over `vectors`
        ...
        logical(c_bool), dimension(n_vectors, n_vectors), intent(out) :: tmp_cloud_masks
        ...
        call clear_err_arg_pos(ierr)
    end subroutine estimate_stc_parameters_expert
