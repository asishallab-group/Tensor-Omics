...
!> summary: Wrappers for [[tox_data_integration_preprocessing_kernel(module)]]
...
module tox_data_integration_preprocessing
    ...
    public :: pool_means
    public :: pool_means_alloc
    public :: pool_study_means
    public :: pool_study_means_alloc
    public :: construct_neighborhoods
    public :: construct_neighborhoods_alloc

contains

    !> summary: Validates its inputs, then calls [[tox_data_integration_preprocessing_kernel(module):compute_gene_means_kernel]].
    ...
    !> summary: Validates its inputs, then calls [[tox_data_integration_preprocessing_kernel(module):compute_residuals_kernel]].
    ...
    !> summary: Validates its inputs, then calls [[tox_data_integration_preprocessing_kernel(module):pool_means_kernel]].
    ...
    !> summary: Allocates its work arrays, then calls [[tox_data_integration_preprocessing_kernel(module):pool_means_kernel]].
    !| This takes the pool already built; `pool_study_means` pools the means of two studies
    !| first, if that is what is at hand.
    subroutine pool_means_alloc(&
            pooled_means,&
            pool_size,&
            n_points,&
            n_pool,&
    ...
    !> summary: Validates its inputs, then calls [[tox_data_integration_preprocessing_kernel(module):pool_study_means_kernel]].
    ...
    !> summary: Allocates its work arrays, then calls [[tox_data_integration_preprocessing_kernel(module):pool_study_means_kernel]].
    !| Concatenates the two studies' means, sorts the pool, and turns it into reference
    !| points exactly as `pool_means` does.
    subroutine pool_study_means_alloc(&
            n_genes_S1,&
            mean_S1,&
            n_genes_S2,&
            mean_S2,&
    ...
    end subroutine pool_study_means_alloc

    !> summary: Validates its inputs, then calls [[tox_data_integration_preprocessing_kernel(module):construct_neighborhoods_kernel]].
    ...
    !> summary: Allocates its work arrays, then calls [[tox_data_integration_preprocessing_kernel(module):construct_neighborhoods_kernel]].
    subroutine construct_neighborhoods_alloc(&
            n_points,&
            x_star,&
            n_genes_S,&
            mean_S,&
