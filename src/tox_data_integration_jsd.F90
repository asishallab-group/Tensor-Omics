...
!         integer(int32) :: n_pool, n_points, n_neighbors, i_study, i_neighbor_count, i_point_count, best_params_CI_i_point_count, best_params_CI_i_neighbor_count, best_params_exceeded_CI_overlap
!         logical :: plateau_found, all_have_min_overlap
!         type(c_ptr) :: rng
!         real(real64), dimension(:), pointer :: confidence_interval_flat

!         rng = create_rng()

!         n_pool = find_last_non_nan(gene_means, gene_means_perm_all, size(gene_means, kind=int32))
...
!             call pool_means_helper(gene_means, gene_means_perm_all, size(gene_means, kind=int32), n_points, n_pool, x_star)
