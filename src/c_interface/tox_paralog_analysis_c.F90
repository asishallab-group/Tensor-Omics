...
contains
    ...
    subroutine detect_dosage_effect_c(&
        ...
        integer(c_int), intent(in), target :: n_mask_chunks
            ...
            !! M_DM_FROM_JUST_INFO to compute this argument using [[tox_paralog_analysis(module):mask_chunk_count]]'s output `count`.
        integer(c_int), intent(in), target :: n_paralog_subsets
            !! number of gene subsets that can be stored in `work_arr_paralog_subsets`.
            !! M_DM_FROM_AUTO to compute this argument using [[tox_paralog_analysis(module):calc_work_arr_paralog_subsets_size]]'s output `work_array_size`.
    ...
    subroutine detect_patterns_c(&
        ...
        real(c_double), intent(in), dimension(n_dims, n_genes), target :: genes
            !! expression vectors of genes
        character(len=1, kind=c_char), intent(in), dimension(15), target :: pattern_mode
        ...
        integer(c_int), intent(out), target :: ierr
            !! error code
        character(len=:), allocatable :: pattern_mode_f
        integer(int32) :: pattern_mode_int_f
        M_CHECK_IERR_NON_NULL
        M_CHECK_NON_NULL(ancestor)
        M_CHECK_NON_NULL(genes)
        M_CHECK_NON_NULL(n_genes)
        ...
        M_CHECK_NON_NULL(active_mask)
        M_CHECK_NON_NULL(temp_paralog_vector)
        call c_char_1d_as_string(pattern_mode, pattern_mode_f, ierr)
        if (is_err(ierr)) return
        select case (pattern_mode_f)
            case ("dosage_pattern")
                    pattern_mode_int_f = MODE_DOSAGE_PATTERN
            case ("subfunc_pattern")
                    pattern_mode_int_f = MODE_SUBFUNC_PATTERN
            case default
        ...
        if (pattern_mode_int_f == MODE_SUBFUNC_PATTERN) then
            call  detect_patterns(&
                ...
                n_genes = n_genes,&
                n_dims = n_dims,&
                pattern_mode = pattern_mode_int_f,&
                filtered_paralogs_mask = filtered_paralogs_mask,&
                n_mask_chunks = n_mask_chunks,&
                n_results = n_results,&
                max_subset_size = max_subset_size,&
