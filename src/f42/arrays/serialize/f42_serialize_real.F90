...
module f42_arrays_serialize_real
    use safeguard
    use, intrinsic :: iso_fortran_env, only: int32, real64
    use f42_arrays_utils, only: write_file_header, REAL_TYPE_CODE
...
contains
    ...
    subroutine serialize_real_helper(arr, n_elements, orig_shape, filename, ierr)
...
end module f42_arrays_serialize_real

!> C binding for the subroutine to serialize a flat real array into a file.
subroutine serialize_real_nd_c(arr, orig_shape, n_dims, &
                                 filename, fn_len, ierr) bind(C, name="serialize_real_nd_c")
    use, intrinsic :: iso_c_binding, only: c_char, c_int, c_double
    use f42_arrays_serialize_real, only: serialize_real_helper
    ...
    real(c_double), dimension(product(orig_shape)), intent(in), target :: arr
