...
!| preprocessing kernels -- into a JSD value. The generator turns the `*_kernel` procedures
...
!| per-family analysis both drive, and carries no `_kernel` suffix.
module tox_data_integration_jsd_kernel
...
contains
    ...
    pure subroutine determine_study_shared_residual_range_kernel(neighborhood_residuals_S1, neighborhood_residuals_S2, n_reps_S1, n_reps_S2, n_neighbors, n_points, &
        ...
        call determine_shared_residual_range_kernel(tmp_abs_residual_pool, tmp_abs_residual_pool_perm, pool_size, shared_residual_range, residual_range_quantile)
    ...
    pure subroutine build_residual_histograms_kernel(neighborhood_residuals, n_reps, n_neighbors, n_points, shared_residual_range, n_bins, counts, pmf, included_n_reps, neighbor_mask)
        ...
        ! Guard against a zero range (e.g. `determine_shared_residual_range_kernel` returns 0.0 when all
    ...
    pure subroutine jct_compute_jsd_pipeline_helper(neighborhood_residuals_S1, neighborhood_residuals_S2, n_reps_S1, n_reps_S2, n_neighbors, n_points, n_bins, shared_residual_range, js_divergences, included_n_reps_S1, included_n_reps_S2, global_js_divergence, weights, pmf_S1, pmf_S2, tmp_counts, neighbor_mask_S1, neighbor_mask_S2)
        ...
        call build_residual_histograms_kernel(neighborhood_residuals_S1, n_reps_S1, n_neighbors, n_points, shared_residual_range, n_bins, tmp_counts, pmf_S1, included_n_reps_S1, neighbor_mask_S1)
        call build_residual_histograms_kernel(neighborhood_residuals_S2, n_reps_S2, n_neighbors, n_points, shared_residual_range, n_bins, tmp_counts, pmf_S2, included_n_reps_S2, neighbor_mask_S2)
        call compute_divergence_per_reference_point_kernel(pmf_S1, pmf_S2, n_points, n_bins, js_divergences)
        call compute_weighted_global_divergence_kernel(js_divergences, n_points, included_n_reps_S1, included_n_reps_S2, global_js_divergence, weights)
    end subroutine jct_compute_jsd_pipeline_helper
end module tox_data_integration_jsd_kernel
