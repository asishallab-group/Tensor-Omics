...
!| The generator turns `euclidean_distance_kernel` and `distance_to_centroid_kernel` into the
!| validating wrappers `euclidean_distance` and `distance_to_centroid` in module
!| `tox_euclidean_distance`.
module tox_euclidean_distance_kernel
...
contains
    ...
    pure subroutine euclidean_distance_kernel(vec1, vec2, n_elements, result)
    ...
    end subroutine euclidean_distance_kernel
    ...
    pure subroutine distance_to_centroid_kernel(n_genes, n_families, genes, centroids, &
        ...
        do concurrent (i_gene = 1:n_genes) local(family_idx) shared(gene_to_fam, n_families, distances, genes, centroids, n_tissues)
            ...
            call euclidean_distance_kernel(genes(:, i_gene), centroids(:, family_idx), n_tissues, distances(i_gene))
        end do
    end subroutine distance_to_centroid_kernel
end module tox_euclidean_distance_kernel
