...
!| other sample.
module tox_trajectory_contribution_analysis_kernel
...
contains
    ...
    !| AUTHOR_FRANZ_ERIC_SILL
    subroutine perform_permutation_test_kernel(trajectories, n_factors, n_samples, n_timepoints, factor_idx, dependent_idx, sample_idx, baseline_mode, n_permutations, local_contributions, total_contributions, tmp_factor, tmp_dependent, random_seed, ierr)
        ...
        do i_perm = 1, n_permutations
            ...
            call compute_contributions_kernel(tmp_factor, tmp_dependent, n_timepoints, baseline_mode, local_contributions(:, i_perm), total_contributions(i_perm), ierr)
            if (is_err(ierr)) return
        end do
    end subroutine perform_permutation_test_kernel
    ...
    pure subroutine compute_p_values_kernel(local_contributions_observed, total_contribution_observed, local_contributions_perm_test, total_contributions_perm_test, n_timepoints, n_permutations, local_p_values, total_p_value)
        integer(int32), intent(in) :: n_timepoints
            !! number of timepoints
        integer(int32), intent(in) :: n_permutations
            !! number of permutations to perform
    ...
    end subroutine compute_p_values_kernel



    !> summary: Performs contribution analysis for a specific factor-dependent pair
    !| AUTHOR_FRANZ_ERIC_SILL
    pure subroutine compute_contributions_kernel(factor, dependent, n_dims, baseline_mode, local_contributions, total_contribution, ierr)
        ...
        call compute_baselines_factor_dependent_kernel(n_dims, factor, dependent, baseline_mode, factor_baseline, dependent_baseline, ierr)
        if (is_err(ierr)) return
    ...
    pure subroutine compute_all_contributions_kernel(trajectories, n_factors, n_samples, n_timepoints, factor_indices, n_selected_factors, dependent_indices, n_selected_dependents, baseline_mode, local_contributions, total_contributions, tmp_factors, tmp_dependent, ierr)
        ...
        do i_sample = 1, n_samples
            ...
            do i_sel_dependent = 1, n_selected_dependents
                ...
                do i_sel_factor = 1, n_selected_factors
                    call compute_contributions_kernel(tmp_factors(:, i_sel_factor), tmp_dependent, n_timepoints, baseline_mode, local_contributions(:, i_sel_factor, i_sel_dependent, i_sample), total_contributions(i_sel_factor, i_sel_dependent, i_sample), ierr)
                    if (is_err(ierr)) return
                end do
            end do
        end do
    ...
    end subroutine compute_baselines_factor_dependent_kernel
    ...
    pure subroutine compute_acceleration_from_velocity_trajectory_kernel(velocity, acceleration, &
        ...
        call compute_velocity_trajectory_kernel(velocity, acceleration, size(velocity, kind=int32))
    end subroutine compute_acceleration_from_velocity_trajectory_kernel

    !> summary: Computes velocity trajectories from position trajectories
    !| AUTHOR_JITU_DABA
    pure subroutine compute_velocity_trajectories_kernel(trajectories, velocity, &
    ...
    end subroutine compute_velocity_trajectories_kernel



    !> summary: Computes acceleration trajectories from velocity trajectories
    !| AUTHOR_JITU_DABA
    pure subroutine compute_acceleration_from_velocity_kernel(velocity, acceleration, &
                                                              n_factors, n_samples, n_timepoints)
    ...
    end subroutine compute_acceleration_from_velocity_kernel
    ...
    !| @endnote
    pure subroutine compute_velocity_acceleration_contributions_kernel(trajectories, n_factors, n_samples, n_timepoints, baseline_mode, &
    ...
    end subroutine compute_velocity_acceleration_contributions_kernel


end module tox_trajectory_contribution_analysis_kernel
