...
module tox_trajectory_normalization_kernel
    ...
    private
    public :: normalize_variable_timeseries_kernel, &
              normalize_single_trajectory_kernel, &
              normalize_all_trajectories_kernel

contains

    !> summary: Normalize a single variable across time using min-max scaling
    !| AUTHOR_AARON_SCHROEDER
    pure subroutine normalize_variable_timeseries_kernel(v, v_norm, n_points, status)
    ...
    end subroutine normalize_variable_timeseries_kernel
    ...
    pure subroutine normalize_single_trajectory_kernel(trajectory, trajectory_norm, n_factors, n_timepoints, status)
    ...
    end subroutine normalize_single_trajectory_kernel
    ...
    pure subroutine normalize_all_trajectories_kernel(trajectories, trajectories_norm, &
        ...
        do i_sample = 1, n_samples
            do i_factor = 1, n_factors
                ...
                call normalize_variable_timeseries_kernel(tmp_series, tmp_series_norm, n_timepoints, status(i_factor, i_sample))

                trajectories_norm(i_factor, i_sample, :) = tmp_series_norm
            end do
        end do
    end subroutine normalize_all_trajectories_kernel

end module tox_trajectory_normalization_kernel
