...
contains
    ...
    pure subroutine detect_neofunctionalization_impl(ancestors, n_families, genes, n_axes, gene_to_fam, n_genes, thresholds, neofunc)
    ...
    pure subroutine detect_patterns_impl(ancestor, genes, n_genes, n_dims, pattern_mode, filtered_paralogs_mask, n_mask_chunks, n_results, max_subset_size, work_arr_paralog_subsets, n_paralog_subsets, tmp_active_mask, tmp_paralog_vector, max_angle, gain_gamma, rdi_threshold, paralog_norms, sorted_paralog_norms_perm, tmp_work_array, ierr)
        ...
        integer(int32), intent(in) :: n_paralog_subsets
            ...
            !! DM_OUTPUT_FROM(work_array_size, calc_work_arr_paralog_subsets_size, tox_paralog_analysis_impl, AUTO)
    ...
    end subroutine detect_patterns_impl
    ...
    pure subroutine filter_paralogs_by_pattern_impl(pattern_mode, gene_angles, threshold, n_genes, n_families, gene_to_fam, masks, n_mask_chunks, ierr)
        integer(int32), intent(in) :: n_genes
            !! number of genes
        integer(int32), intent(in) :: n_families
            !! number of families
        ...
        integer(int32), intent(in) :: pattern_mode
            ...
            !! |         Mode         |                                Value                                |                  Procedure                     |
            !! |----------------------|---------------------------------------------------------------------|------------------------------------------------|
            !! |    Dosage Effect     |  [[tox_paralog_analysis_impl(module):MODE_DOSAGE_PATTERN(variable)]]   | filter_paralogs_by_pattern_dosage_effect       |
            !! | Subfunctionalization |  [[tox_paralog_analysis_impl(module):MODE_SUBFUNC_PATTERN(variable)]]  | filter_paralogs_by_pattern_subfunctionalization |
            !!
    ...
    end subroutine filter_paralogs_by_pattern_impl
