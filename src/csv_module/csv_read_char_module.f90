!> @brief A module to extract specified columns from a 2D string array into another 2D string array.
...
CONTAINS
    ...
    !> @brief Extracts specified columns from a 2D string array.
    SUBROUTINE read_character_columns(data_in, cols_to_read, char_data_out, status)
        ...
        DO j = 1, num_cols_out
            ...
            IF (col_idx < 1 .OR. col_idx > num_cols_in) THEN
                ...
                char_data_out(:, j) = "" ! Fill with empty strings
                CYCLE
            END IF
            
            ! Direct array slice assignment since no type conversion is needed.
...
!> @brief C interface for reading character columns from a flat character array.
...
!> @brief R interface for reading character columns.
