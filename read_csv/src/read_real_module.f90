!> @brief A module to convert specified columns from a 2D string array into a 2D real array.
...
CONTAINS
    ...
    !> @brief Reads specified columns from a string array and converts them to reals.
    SUBROUTINE read_real_columns(data_in, cols_to_read, real_data_out, status)
        ...
        DO j = 1, num_cols_out
            ...
            DO i = 1, num_rows
                READ(data_in(i, col_idx), *, IOSTAT=read_stat) real_data_out(i, j)
                IF (read_stat /= 0) THEN
...
!> @brief C interface for reading real columns from a flat character array.
...
!> @brief R interface for reading real columns.
