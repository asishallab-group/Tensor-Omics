!> @brief Unit test suite for csv_file_reader_module.
MODULE mod_test_csv_file_reader
    ...
    USE asserts, ONLY: assert_true, assert_equal_int, assert_string_equal, assert_equal_size
...
CONTAINS
    ...
    !> @brief Helper to create a dummy file for testing.
    ...
    !> @brief Helper to delete a dummy file after testing.
    SUBROUTINE delete_dummy_file(filename)
        CHARACTER(LEN=*), INTENT(IN) :: filename
        ! CORRECTED: Added declarations for local variables.
        LOGICAL :: ex
        INTEGER :: ierr
    ...
    !> @brief Get array of all available tests for this suite.
    ...
    !> @brief Test basic CSV file reading with a header.
    SUBROUTINE test_read_basic_csv()
        ...
        CALL assert_equal_size(SIZE(header), 3, "test_read_basic_csv: incorrect header size")
        CALL assert_equal_size(SIZE(data, DIM=1), 2, "test_read_basic_csv: incorrect number of rows")
        CALL assert_equal_size(SIZE(data, DIM=2), 3, "test_read_basic_csv: incorrect number of columns")
