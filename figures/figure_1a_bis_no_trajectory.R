...
pdf('figure_1a_bis_no_trajectory.pdf', width = 8, height = 6)
...
for (i in 0:6){
    segments(x0 = .6, y0 = i/2 + .6, x1 = 4.5, y1 = i/2 + .6, col = "grey", lwd = .8)
}

lines(c(.6, .6), c(.6, 4), col = "black", lwd = 1.5)              
lines(c(.6, 4.5), c(.6, .6), col = "black", lwd = 1.5)              

# Tissue labels
text(0, 3.9, label = TeX('\\textit{leaf}'), pos = 4, offset = .2, cex = 1.0)
text(3.9, .3, label = TeX('\\textit{root}'), pos = 3, offset = .1, cex = 1.0)
