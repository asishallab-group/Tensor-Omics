...
contains
    ...
    subroutine test_density_labels_basic()
        ...
        ! New required workspace for calculate_labels_as_density(...)
        integer(int32) :: tmp_stack(3, 64, n_vecs)

        ! Workspaces to build a structurally valid KD-Tree
        ...
        ! Construct KD-Tree index layout
        ...
        ! 2. Act (updated signature includes tmp_stack)
        ...
        do i_chk = 1, 6
            call assert_equal_real(label_densities(i_chk), 6.0_real64, 0.0_real64, &
                                   "Dense cluster tracking verification")
        end do

        ! Members 7 to 10 should find exactly 1 neighbor (only itself)
        do i_chk = 7, 10
            call assert_equal_real(label_densities(i_chk), 1.0_real64, 0.0_real64, &
                                   "Isolated point density verification")
        end do
