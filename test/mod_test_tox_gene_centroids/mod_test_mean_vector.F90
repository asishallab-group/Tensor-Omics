!> The `mean_vector` cases: the element-wise mean of the columns a logical mask selects, the
!| selection at its bounds (first gene, last gene, every gene), the empty selection and its
!| documented zero vector, cancellation, extreme magnitudes, the dimension checks, the check of
!| n_selected_genes against the mask, and the NaN/Inf check.
!|
...
module mod_test_mean_vector
    ...
    !> A gene the mask selects.
    logical(c_bool), parameter :: SELECTED = .true._c_bool
    !> A gene the mask leaves out.
...
contains
    ...
    function get_all_tests_mean_vector() result(all_tests)
        ...
        all_tests(5) = test_case("test_mean_vector_cancellation", test_mean_vector_cancellation)
        all_tests(6) = test_case("test_mean_vector_extreme_magnitudes", test_mean_vector_extreme_magnitudes)
        all_tests(7) = test_case("test_mean_vector_dimensions", test_mean_vector_dimensions)
        all_tests(8) = test_case("test_mean_vector_n_selected_genes_below_zero", &
                                 test_mean_vector_n_selected_genes_below_zero)
        all_tests(9) = test_case("test_mean_vector_n_selected_genes_mismatch", test_mean_vector_n_selected_genes_mismatch)
        all_tests(10) = test_case("test_mean_vector_rejects_nan_and_inf", test_mean_vector_rejects_nan_and_inf)
        all_tests(11) = test_case("test_mean_vector_single_axis_and_gene", test_mean_vector_single_axis_and_gene)
    end function get_all_tests_mean_vector

    !> Four of five genes: every gene but gene 3, which holds 1000 on every axis, so including
    !| it would show. Per axis, over genes 1, 2, 4, 5:
    !| (1 + 3 + 5 + 7)/4 = 4, (-2 + 6 - 10 + 2)/4 = -1, (8 + 0.5 + 2 + 1.5)/4 = 3.
    subroutine test_mean_vector_values()
        integer(int32), parameter :: n_axes = 3, n_genes = 5
        real(real64) :: vectors(n_axes, n_genes), centroid(n_axes), expected(n_axes)
        logical(c_bool) :: genes_selection_mask(n_genes)
        integer(int32) :: ierr
        ...
        genes_selection_mask = [SELECTED, SELECTED, LEFT_OUT, SELECTED, SELECTED]
        expected = [4.0_real64, -1.0_real64, 3.0_real64]
        centroid = UNWRITTEN

        call mean_vector(vectors, n_axes, n_genes, genes_selection_mask, 4, centroid, ierr)
    ...
    !> The selection at its bounds: the first gene alone, the last alone, and every gene (an
    !| all-.true. mask, n_selected_genes = n_genes). A single gene's mean is that gene's vector,
    !| bit for bit, even for values like 12.3 that no double holds exactly: x/1 is exact.
    subroutine test_mean_vector_selection_bounds()
        integer(int32), parameter :: n_axes = 2, n_genes = 4
        real(real64) :: vectors(n_axes, n_genes), centroid(n_axes), expected(n_axes)
        logical(c_bool) :: first(n_genes), last(n_genes), every(n_genes)
        integer(int32) :: ierr

        first = [SELECTED, LEFT_OUT, LEFT_OUT, LEFT_OUT]
        last = [LEFT_OUT, LEFT_OUT, LEFT_OUT, SELECTED]
        every = SELECTED
    ...
    !> A count that is not a power of two: all three genes per axis average to
    ...
    subroutine test_mean_vector_three_genes()
        ...
        call mean_vector(vectors, n_axes, n_genes, genes_selection_mask, n_genes, centroid, ierr)
    ...
    !> An all-.false. mask, n_selected_genes = 0, the smallest valid count: the documented
    !| result of a selection without genes is the zero vector, with ierr = ERR_OK. The genes all
    !| hold 5, so a centroid of any of them would show.
    subroutine test_mean_vector_no_genes()
        ...
        call mean_vector(vectors, n_axes, n_genes, genes_selection_mask, 0, centroid, ierr)
    ...
    subroutine test_mean_vector_dimensions()
        ...
        logical(c_bool) :: genes_selection_mask(2)
        integer(int32) :: ierr

        vectors = 1.0_real64
        genes_selection_mask = [SELECTED, LEFT_OUT]

        call mean_vector(vectors, 0, 2, genes_selection_mask, 1, centroid, ierr)
        call assert_err(ierr, ERR_EMPTY_INPUT, "test_mean_vector_dimensions: n_axes = 0", 2)
        call mean_vector(vectors, -1, 2, genes_selection_mask, 1, centroid, ierr)
        call assert_err(ierr, ERR_INVALID_INPUT, "test_mean_vector_dimensions: n_axes = -1", 2)
        ! n_selected_genes = 0 here: with n_genes < 1 the mask is empty, so 0 is its only
        ! consistent count and the dimension is the one thing wrong.
        call mean_vector(vectors, 2, 0, genes_selection_mask, 0, centroid, ierr)
        call assert_err(ierr, ERR_EMPTY_INPUT, "test_mean_vector_dimensions: n_genes = 0", 3)
        call mean_vector(vectors, 2, -1, genes_selection_mask, 0, centroid, ierr)
        call assert_err(ierr, ERR_INVALID_INPUT, "test_mean_vector_dimensions: n_genes = -1", 3)
    end subroutine test_mean_vector_dimensions

    !> n_selected_genes (argument 5) = -1, below its minimum of 0, is ERR_INVALID_INPUT. The
    !| mask selects nothing, so -1 is also one less than its count: both checks blame
    !| argument 5 with the same code. 0 itself is in `test_mean_vector_no_genes`.
    subroutine test_mean_vector_n_selected_genes_below_zero()
        ...
        call mean_vector(vectors, n_axes, n_genes, genes_selection_mask, -1, centroid, ierr)
        call assert_err(ierr, ERR_INVALID_INPUT, "test_mean_vector_n_selected_genes_below_zero: -1", 5)
    end subroutine test_mean_vector_n_selected_genes_below_zero

    !> n_selected_genes (argument 5) must be the count of .true. in genes_selection_mask: one
    !| more or one less is ERR_INVALID_INPUT, blaming argument 5. The mask [T, F, T] counts 2,
    !| so 3 and 1 are wrong; 3 is also n_genes, a count no longer invalid by itself. The
    !| selected genes sit first and last, so the count must cover the whole mask.
    subroutine test_mean_vector_n_selected_genes_mismatch()
        integer(int32), parameter :: n_axes = 1, n_genes = 3
        real(real64) :: vectors(n_axes, n_genes), centroid(n_axes)
        logical(c_bool) :: genes_selection_mask(n_genes)
        integer(int32) :: ierr
        ...
        call mean_vector(vectors, n_axes, n_genes, genes_selection_mask, 3, centroid, ierr)
        call assert_err(ierr, ERR_INVALID_INPUT, "test_mean_vector_n_selected_genes_mismatch: count + 1", 5)
        call mean_vector(vectors, n_axes, n_genes, genes_selection_mask, 1, centroid, ierr)
        call assert_err(ierr, ERR_INVALID_INPUT, "test_mean_vector_n_selected_genes_mismatch: count - 1", 5)
    end subroutine test_mean_vector_n_selected_genes_mismatch

    !> NaN, +Inf and -Inf in expression_vectors (argument 1) are ERR_NAN_INF. Each sits in the
    !| last element, of a gene the mask leaves out: the whole matrix is checked.
    ...
    subroutine test_mean_vector_single_axis_and_gene()
        ...
        logical(c_bool) :: genes_selection_mask(1)
        integer(int32) :: ierr
        ...
        call mean_vector(vectors, 1, 1, genes_selection_mask, 1, centroid, ierr)
