...
!|
!| Each field holds [origin, shift], the layout `compute_shift_vector_field` stores: a shift
!| vector s = p - o starts at its family's centroid o. The angle turns the origin o to the point
!| the shift reaches, o + s (the paralog p) -- not to s itself, which is how the origin-to-
!| paralog case tells the two readings apart. Every shift below is its target minus its origin,
!| derived by hand and chosen so that o + s is exact.
!|
...
contains
    ...
    function get_all_tests_clock_hand_angles_for_shift_vectors() result(all_tests)
        ...
        all_tests(1) = test_case("test_clock_hand_angles_for_shift_vectors_origin_plus_shift", &
                                 test_clock_hand_angles_for_shift_vectors_origin_plus_shift)
        all_tests(2) = test_case("test_clock_hand_angles_for_shift_vectors_axis_turns", &
                                 test_clock_hand_angles_for_shift_vectors_axis_turns)
        all_tests(3) = test_case("test_clock_hand_angles_for_shift_vectors_one_reference", &
                                 test_clock_hand_angles_for_shift_vectors_one_reference)
        all_tests(4) = test_case("test_clock_hand_angles_for_shift_vectors_selection_mask", &
                                 test_clock_hand_angles_for_shift_vectors_selection_mask)
        all_tests(5) = test_case("test_clock_hand_angles_for_shift_vectors_unoriented_field", &
                                 test_clock_hand_angles_for_shift_vectors_unoriented_field)
        all_tests(6) = test_case("test_clock_hand_angles_for_shift_vectors_dimensions", &
                                 test_clock_hand_angles_for_shift_vectors_dimensions)
        all_tests(7) = test_case("test_clock_hand_angles_for_shift_vectors_mask_count", &
                                 test_clock_hand_angles_for_shift_vectors_mask_count)
        all_tests(8) = test_case("test_clock_hand_angles_for_shift_vectors_rejects_nan_inf", &
                                 test_clock_hand_angles_for_shift_vectors_rejects_nan_inf)
        all_tests(9) = test_case("test_clock_hand_angles_for_shift_vectors_zero_vector", &
                                 test_clock_hand_angles_for_shift_vectors_zero_vector)
    end function get_all_tests_clock_hand_angles_for_shift_vectors

    !> The contract itself: the angle turns the origin to origin + shift, not to the shift. With
    !| the reference e2, the origin e1 and the shift e2 - e1 = (-1, 1) reach e2, so +PI/2 (the
    !| angle from e1 to the shift itself would be 3PI/4). The shift -e1/2 runs back along the
    !| origin without passing zero, reaching e1/2: no turn, so 0 (to the shift itself: PI). And a
    !| zero shift -- the paralog sits on its centroid -- reaches the origin itself: 0 again, not
    !| the zero vector that would fail the call.
    subroutine test_clock_hand_angles_for_shift_vectors_origin_plus_shift()
        real(real64) :: fields(2, 2, 2), reference(2), signed_angles(2), expected(2)
        logical(c_bool) :: fields_selection_mask(2)
        integer(int32) :: ierr

        fields(:, 1, 1) = [1.0_real64, 0.0_real64]
        fields(:, 2, 1) = [-1.0_real64, 1.0_real64]
        fields(:, 1, 2) = [1.0_real64, 0.0_real64]
        fields(:, 2, 2) = [-0.5_real64, 0.0_real64]
        fields_selection_mask = .true.
        reference = [0.0_real64, 1.0_real64]
        expected = [PI/2, 0.0_real64]

        call clock_hand_angles_for_shift_vectors(fields, 2, 2, fields_selection_mask, 2, reference, signed_angles, ierr)
        call assert_equal_int(get_err_code(ierr), ERR_OK, "test_clock_hand_angles_for_shift_vectors_origin_plus_shift: ierr")
        call assert_equal_array_real(signed_angles, expected, 2, ANGLE_TOL, &
                                     "test_clock_hand_angles_for_shift_vectors_origin_plus_shift: e1 -> e2, e1 -> e1/2")

        fields(:, 2, 2) = 0.0_real64
        call clock_hand_angles_for_shift_vectors(fields, 2, 2, fields_selection_mask, 2, reference, signed_angles, ierr)
        call assert_err(ierr, ERR_OK, "test_clock_hand_angles_for_shift_vectors_origin_plus_shift: zero shift")
        call assert_equal_real(signed_angles(2), 0.0_real64, ANGLE_TOL, &
                               "test_clock_hand_angles_for_shift_vectors_origin_plus_shift: a zero shift is no turn")
    end subroutine test_clock_hand_angles_for_shift_vectors_origin_plus_shift

    !> Three fields from e1, with the reference e2 (counter-clockwise positive): the shift
    !| (-1, 1) reaches e2, so +PI/2; (-2, 0) reaches -e1, so PI (antiparallel, so unsigned);
    !| (-1, -1) reaches -e2, so -PI/2.
    subroutine test_clock_hand_angles_for_shift_vectors_axis_turns()
        ...
        fields(:, 2, 1) = [-1.0_real64, 1.0_real64]
        fields(:, 1, 2) = [1.0_real64, 0.0_real64]
        fields(:, 2, 2) = [-2.0_real64, 0.0_real64]
        fields(:, 1, 3) = [1.0_real64, 0.0_real64]
        fields(:, 2, 3) = [-1.0_real64, -1.0_real64]
        fields_selection_mask = .true.
        reference = [0.0_real64, 1.0_real64]
    ...
    !| reference e3: e1 shifted by (-1, 0, 1) reaches e3, +PI/2 (the turn leaves e1 along e3); e2
    !| shifted by (0, -1, -1) reaches -e3, -PI/2 (along -e3); e1 shifted by (-1/2, 0, -sqrt(3)/2)
    !| reaches (1/2, 0, -sqrt(3)/2), with the exact cosine 1/2 and leaving along -e3, so -PI/3;
    !| and e3 shifted by (0, 0, -2) reaches -e3, antiparallel, so PI unsigned.
    subroutine test_clock_hand_angles_for_shift_vectors_one_reference()
        ...
        fields(:, 2, 1) = [-1.0_real64, 0.0_real64, 1.0_real64]
        fields(:, 1, 2) = [0.0_real64, 1.0_real64, 0.0_real64]
        fields(:, 2, 2) = [0.0_real64, -1.0_real64, -1.0_real64]
        ...
        fields(:, 2, 4) = [0.0_real64, 0.0_real64, -2.0_real64]
        fields_selection_mask = .true.
    ...
    !| order: e1 shifted by (-2, 0) reaches -e1, so PI; e1 shifted by (1/sqrt(2) - 1, 1/sqrt(2))
    !| reaches (1, 1)/sqrt(2), so PI/4 (cosine sqrt(2)/2, carrying sqrt's rounding, which acos's
    !| slope -sqrt(2) keeps within an ulp or two; 1/sqrt(2) - 1 and adding 1 back are exact).
    !| Fields 1 and 3 shift e2 by (1, -1) to e1, which the reference e2 cannot orient (the turn
    !| leaves e2 along e1, and e2 . e1 = 0): had either been computed, the call would report
    !| ERR_INVALID_INPUT.
    subroutine test_clock_hand_angles_for_shift_vectors_selection_mask()
        ...
        fields(:, 2, 4) = [1.0_real64, 1.0_real64]/sqrt(2.0_real64) - [1.0_real64, 0.0_real64]
    ...
    !> A single selected field the reference cannot orient fails the call: fields 1 and 3 shift
    !| e1 by (-1, 1, 0) to e2, which the reference e2 orients, but field 2 shifts it by
    !| (-1, 0, 1) to e3, normal to e2. ERR_INVALID_INPUT blames no argument: it is the reference
    !| with this rotation that fails.
    ...
    !| one dimension, one field, one selected: (1) shifted by (-2) reaches (-1), so PI.
    subroutine test_clock_hand_angles_for_shift_vectors_dimensions()
        ...
        call assert_equal_real(signed_angles(1), PI, ANGLE_TOL, &
                               "test_clock_hand_angles_for_shift_vectors_dimensions: (1) + (-2) is (-1)")
    ...
    !> NaN, +Inf and -Inf are ERR_NAN_INF: in a selected field's origin or shift, blaming fields
    ...
    subroutine test_clock_hand_angles_for_shift_vectors_rejects_nan_inf()
        ...
        do i_bad = 1, size(bad)
            ...
            call clock_hand_angles_for_shift_vectors(fields, 2, 2, fields_selection_mask, 1, reference, signed_angles, ierr)
            call assert_err(ierr, ERR_NAN_INF, "test_clock_hand_angles_for_shift_vectors_rejects_nan_inf: shift", 1_int32)
            fields(1, 2, 1) = -1.0_real64
    ...
    !> A selected field has no turn to angle, and fails the call with ERR_DIVISION_BY_ZERO, when
    !| its origin is zero, or when its shift is minus its origin, so that origin + shift is zero:
    !| e1 shifted by (-1, 0) reaches the zero vector. The same field unselected is not angled, so
    !| the call succeeds, and the selected field 2, e1 shifted by (-1, 1) to e2 under the
    !| reference e2, is +PI/2.
    subroutine test_clock_hand_angles_for_shift_vectors_zero_vector()
        ...
        fields(:, 1, 2) = [1.0_real64, 0.0_real64]
        fields(:, 2, 2) = [-1.0_real64, 1.0_real64]
        reference = [0.0_real64, 1.0_real64]
        ...
        call clock_hand_angles_for_shift_vectors(fields, 2, 2, fields_selection_mask, 2, reference, signed_angles, ierr)
        call assert_err(ierr, ERR_DIVISION_BY_ZERO, &
                        "test_clock_hand_angles_for_shift_vectors_zero_vector: origin + shift is zero")
