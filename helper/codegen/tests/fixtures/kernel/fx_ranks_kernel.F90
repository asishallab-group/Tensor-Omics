...
!| author: A Developer
!| One kernel exercising a documented minimum, a work array and a permutation, so that both
!| generated wrappers are meaningful. Compiled and run by `test_end_to_end_fortran.py`.
module fx_ranks_kernel
    use, intrinsic :: iso_fortran_env, only: real64, int32
    M_IMPLICIT_NONE
    private

    public :: rank_scores_kernel

contains

    !> summary: Shifts the scores by `min_score` and returns them in ascending order
    !| author: A Developer
    pure subroutine rank_scores_kernel(n_scores, scores, scores_perm, tmp_shifted, min_score, ranked)
    ...
    end subroutine rank_scores_kernel

end module fx_ranks_kernel
