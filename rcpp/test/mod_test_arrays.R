...
.serialize_nd <- function(helper, arr, filename) {
  ...
  helper(as.vector(arr), as.integer(shape), filename)
}
...
.deserialize_nd <- function(helper, filename) {
  shape <- .file_shape(filename)
  flat <- helper(as.integer(prod(shape)), shape, filename)
  if (length(shape) > 1L) dim(flat) <- shape
  flat
}
...
test_character_serialization <- function() {
  ...
  .serialize_nd(serialize_char_helper, arr1c, fn("char1d.test.bin"))
  ...
  .serialize_nd(serialize_char_helper, arr_ascii1, fn("char_ascii1d.test.bin"))
  ...
  .serialize_nd(serialize_char_helper, arr_ascii2, fn("char_ascii2d.test.bin"))
  ...
  .serialize_nd(serialize_char_helper, arr_ascii3, fn("char_ascii3d.test.bin"))
