...
// [[Rcpp::export]]
List quantile_normalization_rcpp(NumericMatrix input) {
    int n_genes = input.nrow();
    int n_tissues = input.ncol();

    int max_stack = std::max(64, 2 * n_genes);  // maximum stack size
...
// [[Rcpp::export]]
List tox_normalize_data_rcpp(NumericMatrix input, IntegerVector group_s, IntegerVector group_c) {
    int n_genes = input.nrow();
    int n_tissues = input.ncol();
...
// [[Rcpp::export]]
List calc_fchange_rcpp(NumericMatrix input, IntegerVector control_cols, IntegerVector cond_cols) {
    int n_genes = input.nrow();
